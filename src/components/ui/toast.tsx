...
export interface NotificationProps {
  ...
  onClose: () => void;
  position?: NotificationPosition; // Position of the toast
  className?: string; // Custom className for the toast container
}
...
// Helper function to get animation variants based on position
const getAnimationVariants = (position?: NotificationPosition) => {
  const isTop = position?.includes('top') ?? true;
  const isLeft = position?.includes('left') ?? false;
  const isRight = position?.includes('right') ?? true;
...
const Notification: React.FC<NotificationProps> = ({ 
  ...
  return (
    ...
    <motion.div
      ...
      className={`relative w-full max-w-sm rounded-xl p-4 backdrop-blur-xl bg-white/15 dark:bg-black/15 border border-gray-300/60 dark:border-gray-700/60 overflow-hidden ring-1 ring-gray-200/40 dark:ring-gray-700/40 drop-shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 ${className || ''}`}
    >
