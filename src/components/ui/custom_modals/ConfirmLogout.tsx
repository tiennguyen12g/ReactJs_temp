...
const defaultDescription = "Are you sure you want to logout? You will need to sign in again to access your account.";
const defalutRemind = "Any unsaved changes will be lost. Make sure to save your work before logging out."
export default function ConfirmLogout({ isModalOpen, setIsModalOpen, onConfirm = () => {}, description = defaultDescription, remindContent = defalutRemind }: ConfirmLogoutProps) {
  const handleConfirm = () => {
    onConfirm();
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Confirm Logout" size="sm">
        <div className="space-y-4">
          ...
          <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-md p-3">
            <p className="text-sm text-orange-700 dark:text-orange-300">{remindContent}</p>
          </div>
